...
interface ResultsDisplayProps {
  ...
  results: { GEMINI: number; OPENAI: number; GROK: number }
}

export function ResultsDisplay({userSelections, storySummary, onRestart, isSummarizing, results}: ResultsDisplayProps) {
  return (
    <Card className="w-full max-w-3xl mx-auto shadow-xl animate-fade-in">
      ...
      <CardContent className="space-y-6">
            ...
            <ul className="space-y-2">
              ...
              {/*))}*/}
                <li className="font-body text-md">
                  <span className="font-semibold text-primary">Gemini:</span> {results.GEMINI}
                </li>
                <li className="font-body text-md">
                  <span className="font-semibold text-primary">Grok:</span> {results.GROK}
                </li>
                <li className="font-body text-md">
                  <span className="font-semibold text-primary">OpenAI:</span> {results.OPENAI}
                </li>
            </ul>
          </ScrollArea>
        </div>
        ...
        {/*  ) : (*/}
        {/*    <p className="font-body text-md text-destructive">The Photo Selector seems to be resting. Unable to generate*/}
        {/*      a summary at this moment.</p>*/}
        {/*  )}*/}
        {/*</div>*/}

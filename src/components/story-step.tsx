  ...
  return (
    <Card className="w-full max-w-3xl mx-auto shadow-xl animate-fade-in">
      <CardHeader className="text-center">
        <CardTitle className="font-headline text-3xl md:text-4xl text-primary">Selection Round {currentStep + 1}</CardTitle>
        <CardDescription className="font-body text-lg text-muted-foreground">
          Round {currentStep + 1} of {totalSteps}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 pt-4">
          {stepContent.images.map((imageOption, index) => (
            <Button
              key={imageOption.id}
              variant="outline"
              ...
              <div className="flex flex-col items-center w-full rounded-lg overflow-hidden shadow-md bg-card">
                <div className="relative w-full aspect-[4/3]">
                  <Image
                    ...
                    alt={`${stepContent.text} image ${index + 1}`}
                    fill
